...
export const getTasks = async (params: TaskQueryParams) => {
  const { data } = await request.get<PaginatedResponse<CrawlerTask>>('/crawler/tasks', { params })
  return data
}

export const createTask = async (taskData: Omit<CrawlerTask, 'id'>) => {
  const { data } = await request.post<CrawlerTask>('/crawler/tasks', taskData)
  return data
}

export const updateTask = async (id: string, taskData: Partial<CrawlerTask>) => {
  const { data } = await request.put<CrawlerTask>(`/crawler/tasks/${id}`, taskData)
  return data
}

export const deleteTask = async (id: string) => {
  await request.delete(`/crawler/tasks/${id}`)
}

export const startTask = async (id: string) => {
  const { data } = await request.post(`/crawler/tasks/${id}/start`)
  return data
}

export const stopTask = async (id: string) => {
  const { data } = await request.post(`/crawler/tasks/${id}/stop`)
  return data
}

export const batchStartTasks = async (ids: string[]) => {
  const { data } = await request.post('/crawler/tasks/batch-start', { ids })
  return data
}

export const batchStopTasks = async (ids: string[]) => {
  const { data } = await request.post('/crawler/tasks/batch-stop', { ids })
  return data
}

// 配置管理相关API
export const getConfig = async (id: string) => {
  const { data } = await request.get<CrawlerConfig>(`/crawler/configs/${id}`)
  return data
}

export const saveConfig = async (id: string, configData: CrawlerConfig) => {
  const { data } = await request.put<CrawlerConfig>(`/crawler/configs/${id}`, configData)
  return data
}

// 数据管理相关API
export const getData = async (params: DataQueryParams) => {
  const { data } = await request.get<PaginatedResponse<CrawlerData>>('/crawler/data', { params })
  return data
}

export const deleteData = async (id: string) => {
  await request.delete(`/crawler/data/${id}`)
}

export const batchDeleteData = async (ids: string[]) => {
  await request.post('/crawler/data/batch-delete', { ids })
}

export const exportData = async (params: DataQueryParams) => {
  const response = await request.get('/crawler/data/export', { 
    params,
    responseType: 'blob'
  })
